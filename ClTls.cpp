...
void ClTls::sendClientHello( void )
{
CharArray charAr;

Int32 lengthOfOuterMessage = 50; // ?

charAr.setSize( lengthOfOuterMessage );

// The first five bytes are the outer TLS record.

charAr.setC( 0, 22 );
                 // Content type is Handshake

// Casting::i32ToByte( lengthOfOuterMessage );

//   TLS version 1.2 is version 3.3 of SSL.
// But this is legacy stuff and it's also
// 3.3 for Tls 1.3.

charAr.setC( 1, 3 );  // Version Major
charAr.setC( 2, 3 );  // Version Minor
charAr.setC( 3, Casting::i32ToByte(
                    lengthOfOuterMessage >> 8 ));
charAr.setC( 4, Casting::i32ToByte(
                    lengthOfOuterMessage ));

// Start of the inner ClientHello message.
charAr.setC( 5, 1 );
      // Message type 1 is a  ClientHello message.

// minus 4?
Int32 lengthOfClientHelloMessage =
                         lengthOfOuterMessage - 4;
charAr.setC( 6] = (byte)(LengthOfClientHelloMessage >> 16);
charAr.setC( 7] = (byte)(LengthOfClientHelloMessage >> 8);
charAr.setC( 8] = (byte)(LengthOfClientHelloMessage);
charAr.setC( 9] = 3;   // Version Major
charAr.setC( 10] = 3;  // Version Minor

// This part for Unix time is not in TLS
// 1.3 because there's no point in having it.
// But it is in TLS 1.2.
//     ECTime RightNow = new ECTime();
//    RightNow.SetToNow();
//    ulong UnixTime = RightNow.ToUnixTime();

charAr.setC( 11] = (byte)(UnixTime >> 24);
charAr.setC( 12] = (byte)(UnixTime >> 16);
charAr.setC( 13] = (byte)(UnixTime >> 8);
charAr.setC( 14] = (byte)(UnixTime);

// These bytes have to be cryptographically random.
// These are used later in generating the
// master secret and keys, etc.
...
//      {
// charAr.setC( Index] = RandomBytes[Count];
  //    Index++;
    //  }
...
charAr.setC( 43] = 0; // Session ID Length is zero. It's not resuming a session here.

// A normal browser sends a lot more cipher suites
// than this so this is usually a lot longer.
//  The algorithm you prefer to use should
// be listed first.  So they are listed in
// order of preference.
charAr.setC( 44] = 0;
               // Cipher Suites Length high byte
charAr.setC( 45] = 4; // Cipher Suites Length
charAr.setC( 46] = 0;
charAr.setC( 47] = 0x35;
...
charAr.setC( 50] = 1; // Compression Methods Length
charAr.setC( 51] = 0; // Compression Method is null.

// This message has no extensions, so that's
// all there is to send.

// CharBuf sendBuf;
// sendBuf.appendCharArray( charAr,
//                     const Int32 howMany );
// Then send it.
}
...
namespace ExampleServer
{
  ...
  // Apparently some implementations go beyond
// this record length limit.
  // The length is a 16 bit number and so it
//could go up to 64K.
